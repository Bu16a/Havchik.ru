...
async function printRecipe(recipeJson) {
    if (
        ...
        Object.keys(recipeJson).length === 0
    ) {
    ...
    if (ingredientsData)
        Object.keys(ingredientsData).forEach((ingredient) => {
            ...
            Object.keys(products).forEach((product) => {
                ...
                if (
                    productLower.indexOf(ingredientLower) !== -1 ||
    ...
    readyButton.addEventListener("click", (event) => {
        ...
        alert(
            'Поздравляем, вы приготовили блюдо! Оно отобразится в разделе "Приготовленные рецепты"',
        );
    });
    ...
    if (recipeJson.energy) recipeContent.appendChild(recipeNutrition);
